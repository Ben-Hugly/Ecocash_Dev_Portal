    ...
    <div className={`${style.GettingStartedbg}`}>
      <div className=" items-center mb-4">
        <div className={`${style.GettingStartedCardBorder}`}>
          <h3
            className={`${style.smallText} pb-3 text-start font-semibold text-[15px]`}
          >
            {title}
          </h3>
          {Icon && (
      ...
      <div className="space-y-3">
        {links.map((link, index) => (
          <a
            key={index}
            href={link.url}
            className={`${style.smallTextBlue}  hover:scale-105`}
          >
            {link.label}
            <AiOutlineRight
              className={`${style.GettingStartedCardRightOutline}`}
            />
          </a>
        ))}
      </div>
